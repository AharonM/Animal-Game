...
void binary_tree::addLeft(node *nd, const string &data) {
    if (nd == 0) {
    	cout<< "Attempt addLeft on null";
    	throw "Attempt addLeft on null";
    }
	
    if (nd->left != 0){
    	cout<< "Attempt addLeft on nd with existing left child\n";
    	throw "Attempt addLeft on nd with existing left child";
    } 
...
string binary_tree::get(node *node){
	if (node == 0) throw "Attempt addRight on null";
	return node->data;
}
